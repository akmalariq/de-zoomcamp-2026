...
spark = SparkSession.builder \
    .master("local[*]") \
    .appName("de-zoomcamp-hw5") \
    .getOrCreate()

spark.sparkContext.setLogLevel("ERROR")

DATA_DIR = os.path.join(os.path.dirname(__file__), "data")
PARQUET_PATH = os.path.join(DATA_DIR, "yellow_tripdata_2024-10.parquet")
ZONE_PATH = os.path.join(DATA_DIR, "taxi_zone_lookup.csv")
OUTPUT_DIR = os.path.join(DATA_DIR, "yellow_2024_10_partitioned")

# Q1: Spark version
print(f"\nQ1 - Spark version: {spark.version}")

# Q2: Repartition to 4 and save as parquet, check avg file size
...
# Q3: Trips starting on Oct 15
df_partitioned = spark.read.parquet(OUTPUT_DIR)
oct15_count = df_partitioned.filter(
    F.to_date("tpep_pickup_datetime") == "2024-10-15"
).count()
print(f"Q3 - Trips on Oct 15: {oct15_count:,}")
...
least_frequent = df_with_zones.groupBy("Zone") \
    ...
    .orderBy("count") \
    .first()
print(f"Q6 - Least frequent pickup zone: {least_frequent['Zone']} ({least_frequent['count']:,} trips)")
